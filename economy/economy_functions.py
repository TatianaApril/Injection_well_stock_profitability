...
def revenue_side(Netback, Q_oil):
    ...
    years = years.astype({'year': np.int64})
...
def expenditure_side(Q_oil, Q_fluid, W,
    ...
    # cost_inj_wells = np.reshape((np.array(W.iloc[:, 2:]) * unit_costs_injection), (-1, 1)) * num_days
...
def select_analogue(df_FPA, row_nan, Qliq_fact, liquid_groups):
    ...
    new_row = df_FPA[df_FPA["liquid_group"] == liquid_group].agg(mean_str)[-9:]
    if len(new_row.shape) >= 1:
        new_row = new_row.squeeze()
    if new_row.isnull().all() and liquid_group != 24:
        while new_row.isnull().all() and liquid_group <= 24:
            liquid_group += 1
            new_row = df_FPA[df_FPA["liquid_group"] == liquid_group][-9:]
            new_row = new_row.agg(mean_str)
            if len(new_row.shape) >= 1:
                new_row = new_row.squeeze()
    if new_row.isnull().all():
        new_row = df_FPA.agg(mean_str)[-9:]
        if len(new_row.shape) >= 1:
            new_row = new_row.squeeze()
    new_row[:6] = row_nan[:6]
