...
def check_wellIntersection(df_intersectionWells, MaxOverlapPercent):
    """
...
def firstRow_of_Wells_geometry(df_WellOneArea, wellNumberInj,
                               coordinateXT1, coordinateYT1, coordinateXT3, coordinateYT3,
    ...
    :param coordinateXT1/YT1/XT3/YT3: название столбцов с координатами скважин в df_WellOneArea
    ...
    for point in range(len(list_startingPoints[0])):
        ...
        listNamesClear = listNamesOnePointWells
        for well in listNamesOnePointWells:
            if well in listNamesClear:
                ...
                if df_intersectionWells.shape[0] != 1:
                    list_dropWell = check_wellIntersection(df_intersectionWells, MaxOverlapPercent)
                    listNamesClear = list(set(listNamesClear) - set(list_dropWell))

        """# график после очистки
        df_OnePoint = df_OnePoint[df_OnePoint.index.isin(listNamesClear)]
        fig, ax = plt.subplots(subplot_kw={'projection': 'polar'})
        for i in range(df_OnePoint["fi_t1"].shape[0]):
        ...
        listNamesFisrtRowWells.extend(listNamesClear)
    listNamesFisrtRowWells = list(set(listNamesFisrtRowWells))
    return listNamesFisrtRowWells


def firstRow_of_Wells_drainageFront(df_WellOneArea, wellNumberInj, coordinateXT1,
                                    coordinateYT1,
                                    coordinateXT3,
