...
def main_script(reservoir_name: str, path_to_mer: str, path_to_prod: str, path_to_inj: str, **kwargs) -> None:
    ...
    for horizon in list_horizons[7:]:
    ...
    for file in output_content:
        ...
        if df_forecasts.shape[1] > 3:
            ...
            df_ratio_liq["ratio"] = df_ratio_liq.apply(lambda row: row[last_data]
                                                                   / sum_Qliq.loc[row['Ячейка']]
                                                        if sum_Qliq.loc[row['Ячейка']].values != 0 else np.nan,
                                                                                                   axis=1).fillna(0)
            df_ratio_oil["ratio"] = df_ratio_oil.apply(lambda row: row[last_data] / sum_Qoil.loc[row['Ячейка']] if sum_Qoil.loc[row['Ячейка']].values != 0 else np.nan,
                                                       axis=1).fillna(0)
